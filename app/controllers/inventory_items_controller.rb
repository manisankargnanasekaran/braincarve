class InventoryItemsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @inventory_item.save
        CompanyInventoryStock.create(:inventory_item_id=>@inventory_item.id, :available_qty=>0)
        format.html { redirect_to inventory_items_url, notice: 'Inventory item was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @inventory_item.update(inventory_item_params)
        format.html { redirect_to inventory_items_url, notice: 'Inventory item was successfully updated.' }
  ...
  private
    ...
    def inventory_item_params
      params.require(:inventory_item).permit!#(:book, :class_level)
    end
end
